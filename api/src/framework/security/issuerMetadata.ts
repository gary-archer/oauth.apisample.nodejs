...
export class IssuerMetadata {
    ...
    public async load(): Promise<void> {
        ...
        try {
            const endpoint = `${this._configuration.oauth.authority}/.well-known/openid-configuration`;
            this._issuer = await OpenIdClient.Issuer.discover(endpoint);
        } catch (e) {

            const handler = new OAuthErrorHandler(this._configuration);
            throw handler.fromMetadataError(e, this._configuration.oauth.authority);
        }
    }
