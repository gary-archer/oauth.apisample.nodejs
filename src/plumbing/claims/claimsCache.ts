...
export class ClaimsCache<TClaims extends CoreApiClaims> {
    ...
    public async addClaimsForToken(accessTokenHash: string, claims: TClaims): Promise<void> {
        ...
        if (secondsToCache > 0) {
            ...
            // Output debug info
            this._traceLogger.debug(`Token to be cached will expire in ${secondsToCache} seconds (hash: ${accessTokenHash})`);
            ...
            this._traceLogger.debug(`Adding token to claims cache for ${secondsToCache} seconds (hash: ${accessTokenHash})`);
